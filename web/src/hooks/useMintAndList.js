...
import {EVENT_ITEM_MINTED, getImpactXItemsEventByType} from "src/util/events"
...
export default function useMintAndList() {
  ...
  const onTransactionSealed = tx => {
    ...
    const event = getImpactXItemsEventByType(tx.events, EVENT_ITEM_MINTED)
  ...
  const mintAndList = () => {
    ...
    executeMintRequest({
      url: publicConfig.apiImpactXItemMintAndList,
      method: "POST",
      data: {
        recipient,
      },
      onSuccess: data => {
        ...
        analytics.track("ImpactX-items-item-minted", {params: {mint: data}})
      },
      onError: () => {
        resetLoading()
      },

...
import {
  EVENT_ITEM_MINTED,
  EVENT_ImpactX_ITEM_DEPOSIT,
  getImpactXItemsEventByType,
...
const newUpdateTransactionPayload = tx => {
  ...
  const depositEvent = getImpactXItemsEventByType(
    tx.data.events,
    EVENT_ImpactX_ITEM_DEPOSIT
  )
  if (!!depositEvent) {
  ...
  const mintEvent = getImpactXItemsEventByType(tx.data.events, EVENT_ITEM_MINTED)
  if (!!mintEvent) {
    return {
      ...tx,

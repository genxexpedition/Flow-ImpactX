...
export default function HeaderMessage() {
  ...
  const checkIsServiceAccountInitialized = () => {
    isAccountInitializedTx(publicConfig.flowAddress).then(data => {
      setIsServiceAccountInitialized(data.ImpactXItems && data.ImpactXItemsMarket)
    })
  }
      ...
      <HeaderContainer>
        {isServiceAccountLoggedIn ? (
          <>
            ...
            the Service Account to mint Donation NFT's.
          </>
        ) : (
          <>
            {currentUser ? (
  ...
  return (
    <HeaderContainer>
      ...
      {/* <span className="mr-3 text-sm">💻</span>Flow-ImpactX is an application

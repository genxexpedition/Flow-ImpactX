...
function initializeImpactXItems(network) {
  if (!network) return envErr();
  return `flow transactions send -o json --network=${network} --signer ${network}-account ./cadence/transactions/ImpactXItems/setup_account.cdc -f flow.json ${
    network !== "emulator" ? `-f flow.${network}.json` : ""
  }`;
}
...
async function cleanupTestnetConfig() {
  ...
  const dbExists = jetpack.exists("./api/ImpactX-items-db-testnet.sqlite");
  if (dbExists) {
    spinner.info(
    ...
    if (removeDb.confirm) {
      ...
      jetpack.remove("./api/ImpactX-items-db-testnet.sqlite");
...
pm2.connect(false, async function (err) {
  ...
  async function deployAndInitialize() {
    ...
    // -------------- Initialize Flow-ImpactX  --------------------------

    spinner.info(`Initializing Flow-ImpactX`);
    const { stderr: err2 } = await exec(
      initializeImpactXItems(process.env.CHAIN_ENV),
      { cwd: process.cwd() }
    );
  ...
  if (process.env.CHAIN_ENV === "emulator") {
    ...
    // emitted by the Flow-ImpactX contract, and the sale offers will match
    // with what is represented on-chain (what NFTs are for sale in which accounts).
    jetpack.remove("./api/ImpactX-items-db-emulator.sqlite");
  ...
  spinner.info(
    `Flow-ImpactX API is running at: ${chalk.yellow("http://localhost:3000")}`
  );
  spinner.info(
  ...
  spinner.info(
    `Flow-ImpactX Web App is running at: ${chalk.yellow(
      "http://localhost:3001"
    )}`
  );
  spinner.info(
  ...
  const rainbow = chalkAnimation.rainbow("Flow-ImpactX HAS STARTED");
